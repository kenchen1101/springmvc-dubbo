...
public class PropertyManager {
    ...
    /**
     * Returns the version number of enet as a String. i.e. -- major.minor.revision
     */
    ...
    /**
     * Gets a enet property. enet properties are stored in enet.properties. The properties file should be accesible from the classpath. Additionally, it should have a path field that gives the full path to where the file is located. Getting properties is
     * a fast operation.
     * 
     * @param name
    ...
    /**
     * Sets a enet property. Because the properties must be saved to disk every time a property is set, property setting is relatively slow.
     */
    ...
    private void saveProps() {
        ...
        try {
            ...
            System.err.println("There was an error writing enet.properties to " + path + ". " + "Ensure that the path exists and that the enet process has permission " + "to write to it -- " + ioe);
            ioe.printStackTrace();
        } finally {
            try {
                out.close();
