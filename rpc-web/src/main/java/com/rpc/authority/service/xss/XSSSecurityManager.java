package com.rpc.authority.service.xss;
...
public class XSSSecurityManager {
     ...
     */
    // public static List<Map<String,Object>> checkUrlMatcherList = new ArrayList();
    ...
    public static boolean initConfig(String path) throws DocumentException {
        ...
        if (checkUrlEles != null) {
            ...
            while (checkUrls.hasNext()) {
                ...
                if (!"".equals(tempCheckUrlKey)) {
                    ...
                    log.error("## 安全过滤配置文件加载失败:特殊url正则表达式异常,url匹配为 ", tempCheckUrlKey);
                    return false;
                }
            }

        } else {
            log.error("## 安全过滤配置文件中没有 {} 属性", XSSSecurityConstants.CHECK_URL_LIST);
            return false;
        }
        ...
        if (regexEle != null) {
            ...
            if (tempStr.charAt(tempStr.length() - 1) == '|') {
                ...
                // 必须配置regex属性
                if (log.isDebugEnabled()) {
                    log.debug("## 安全过滤配置文件加载失败:正则表达式异常{}", tempStr);
                }
                return false;
            }
        } else {
            log.error("## 安全过滤配置文件中没有 {} 属性", XSSSecurityConstants.REGEX_LIST);
            return false;
        }
        if (log.isDebugEnabled()) {
